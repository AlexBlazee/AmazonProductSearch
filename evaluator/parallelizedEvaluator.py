...
class ParallelizedEvaluator():

	def process_query(self , data, query_id , engine):
		...
		recommended_doc_ids = recommendations['product_id'].to_list()
		recommended_scores = recommendations['scores'].to_list()
		return relevant_doc_ids, relevant_score, recommended_doc_ids, recommended_scores

	def evaluate_rank_metric(self , query_ids, data, engine , TOP_VAL=None):
		...
		with ThreadPoolExecutor() as executor:
			futures = [executor.submit(self.process_query, data, query_ids[i] , engine ) for i in range(Iter_val)]
